<template>
  <el-header class="header" style="::shadow ">
    ...
    <div class="user">
      ...
      <el-icon style="margin-right: 0px" @click="changeTheme()">
        <sunny v-if="theme === 'light'" />
        <moon v-else />
      </el-icon>
      <el-badge :hidden="have_dot" is-dot style="margin: 0px 10px !important">
        <bell
          style="width: 1em; height: 1em; margin-bottom: 4px"
          v-if="!loggedIn"
          @click="messages()"
      /></el-badge>
      <el-drawer v-model="drawer" title="消息列表" direction="rtl">
        <el-scrollbar full>
      ...
      <el-icon @click="$router.push('/user/' + (loggedIn ? 'edit' : 'login'))">
